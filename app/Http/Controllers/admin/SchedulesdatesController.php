...
class SchedulesdatesController extends Controller
{
    ...
    {
        try {
            $maintenances = DB::select('CALL sp_maintenaces()');

            $maintenances = collect($maintenances)->map(function ($maintenance) {
                ...
                $maintenance->date = Carbon::parse($maintenance->date)->format('d/m/Y');
                $maintenance->time_start = Carbon::parse($maintenance->time_start)->format('H:i');
                $maintenance->time_end = Carbon::parse($maintenance->time_end)->format('H:i');
                return $maintenance;
            });


        if ($request->ajax()) {
            return DataTables::of($maintenances)->make(true);
        } else {
            return view('admin.schedulesdates.index', compact('maintenances'));
        }
        } catch (\Throwable $th) {
