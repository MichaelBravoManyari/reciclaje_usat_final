...
class VehicleocuppantsController extends Controller
{
    ...
    {
        try {
            // Obtiene los datos del vehículo y del usuario
            ...
            $existingOccupant = Vehicleocuppants::where('user_id', $id)
                ->where('status', '1')
                ->first();
        
            if ($existingOccupant) {
                return response()->json([
                    'message' => 'El usuario ya está asignado a otro vehículo.',
                    'user_id' => $id,
            ...
            $existingOccupantV = Vehicleocuppants::where('user_id', $id)
                ->where('status', '1')
                ->where('vehicle_id', $vehicle_id)
                ->first();
        
            if ($existingOccupantV) {
                return response()->json(['message' => 'El usuario ya está asignado a este vehículo.'], 409);
            }
            ...
            if ($activeOccupants >= $vehicleCapacity) {
                return response()->json([
                    'message' => 'No se puede asignar más ocupantes. La capacidad máxima del vehículo es de ' . $vehicleCapacity . ' personas.',
                ], 400);
            }
        
            // Registra la asignación
            $newOccupant = new Vehicleocuppants();
