        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($users)
                ->addColumn('logo', function ($user) {
                    return '<img src="' . ($user->logo == '' ? asset('storage/user_logo/no_image.png') : asset($user->logo)) . '" width="80px" height="80px" class="card">';
                })
                ->addColumn('actions', function ($user) {
                    return '
                        <div class="dropdown">
 ...
 */
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request->logo != '') {
                $image = $request->file('logo')->store('public/user_logo');
            ...
            User::create([
                ...
                'profile_photo_path' => $logo,
                'usertype_id' => $request->usertype_id
            ]);

            return response()->json(['message' => 'Personal registrado exitosamente'], 200);
        } catch (\Throwable $th) {
            return response()->json(['message' => 'Error en el registro: ' . $th->getMessage()], 500);
        }
    }
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            $data = $request->except(['password', 'logo']); // Excluir el campo password por defecto
            ...
            if ($request->hasFile('logo') && $request->file('logo')->isValid()) {
                $image = $request->file('logo')->store('public/user_logo');
                $logo = Storage::url($image);
                $data['profile_photo_path'] = $logo;
            }
            
            $user->update($data);

            return response()->json(['message' => 'Personal actualizado exitosamente'], 200);
        } catch (\Throwable $th) {
            return response()->json(['message' => 'Error actualizar: ' . $th->getMessage()], 500);
        }
    }
